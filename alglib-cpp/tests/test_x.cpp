...
public:
    ...
    std::vector<int> running_callbacks_indexes;
    alglib_impl::ae_lock lock;
    bool parallelism_detected;
    bool bad_callback_indexes;
    
    paracbck_rosenbrock_problem(int _n, double _p):n(_n),parameter(_p),countdown(0),raise_aperror(true),delay(10),callbacks_running(0),parallelism_detected(false),bad_callback_indexes(false)
    {
        memset(&lock, 0x0, sizeof(lock));
        alglib_impl::ae_init_lock(&lock, NULL, ae_false);
        running_callbacks_indexes.resize(alglib::getmaxnworkers(),0);
    }
...
void paracbck_rosenbrock_func(const alglib::real_1d_array &vars, double &func, void *ptr)
{
    ...
    //
    alglib::ae_int_t maxnw = alglib::getmaxnworkers();
    if( maxnw<1 || maxnw>1000000 )
        throw std::string("ERROR: alglib::getmaxnworkers() is outside of the feasible range");
#if defined(_ALGLIB_HAS_THREADLOCAL)
    alglib::ae_int_t cbck_idx = alglib::getcallbackworkeridx();
    if( cbck_idx<0 || cbck_idx>=maxnw )
        throw std::string("ERROR: alglib::getcallbackworkeridx() is outside of the feasible range");
#endif
    alglib_impl::ae_int_t t0 = alglib_impl::ae_tickcount();
    ae_acquire_lock(&problem.lock);
    problem.callbacks_running++;
#if defined(_ALGLIB_HAS_THREADLOCAL)
    problem.running_callbacks_indexes[cbck_idx]++;
#endif
    ae_release_lock(&problem.lock);
...
public:
    ...
    paracbck_lsfit_f0_problem(int params, int coord):ncoord(coord),nparams(params),countdown(0),raise_aperror(true),delay(10),callbacks_running(0),parallelism_detected(false),bad_callback_indexes(false)
    {
...
public:
    ...
    paracbck_minlm_f0_problem(int params, int ndup):nparams(params),nfunc(nparams*ndup),countdown(0),raise_aperror(true),delay(10),callbacks_running(0),parallelism_detected(false),bad_callback_indexes(false)
    {
        alglib::hqrndstate rs;
        alglib::real_1d_array c;
...
#error Unable to determine OS, unexpected here
    ...
    printf("Issues:\n");
    {
        ...
        try
        {
            ...
            alglib_impl::ae_shared_pool_set_seed(&pool, &seed, sizeof(seed), _seedrec_init_copy, _seedrec_destroy, &_alglib_env_state);
    ...
    fflush(stdout);
    {
        ...
        for(int global_cb_parallelism=0; global_cb_parallelism<=2; global_cb_parallelism++)
            for(int local_cb_parallelism=0; local_cb_parallelism<=2; local_cb_parallelism++)
            {
                ...
                {
                    printf(">>> MINLBFGS: alglib::getcallbackworkeridx() is broken\n");
                    test_failed =  true;
                }
                if( f>ftol_lbfgs )
                {
            ...
            for(int cbtype=0; cbtype<=1; cbtype++)
                for(int ndup=1; ndup<=2; ndup++)
                    {   
                        ...
                        if( problem0.bad_callback_indexes )
                        {
                            printf(">>> MINLM: alglib::getcallbackworkeridx() is broken\n");
                            test_failed =  true;
                        }
                    }
            ...
            for(int cbtype=0; cbtype<=1; cbtype++)
                for(int ndup=1; ndup<=2; ndup++)
                    for(int stype=-1; stype<=+1; stype++)
                    {
                        ...
                        if( problem0.bad_callback_indexes )
                        {
                            printf(">>> LSFIT: alglib::getcallbackworkeridx() is broken\n");
                            test_failed =  true;
                        }
                    }
